...
export async function POST(request:Request){
  ...
  const event=await provider.verifyWebhook(payload,null);if(event.kind!=="payment")return NextResponse.json({error:"ประเภท webhook ทดสอบไม่ถูกต้อง"},{status:400});const hash=await crypto.subtle.digest("SHA-256",new TextEncoder().encode(payload)).then(x=>[...new Uint8Array(x)].map(b=>b.toString(16).padStart(2,"0")).join(""));
