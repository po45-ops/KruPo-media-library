...
export async function getAdminSectionRows(section:string):Promise<string[][]>{
  ...
  switch(section){
    ...
    case "orders":{const {data,error}=await db.from("orders").select("id,user_id,total_satang,status,payment_provider,created_at").order("created_at",{ascending:false}).limit(100);if(error)throw error;return (data??[]).map(row=>[row.id.slice(0,8),row.user_id.slice(0,8),baht(row.total_satang),row.payment_provider??"—",row.status,row.id])}
